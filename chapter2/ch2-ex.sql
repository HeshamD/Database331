...
/*
    from the orders table show the emp id
    with counting the ordersNumbers 
    that the order date smaller than 2016/05/01
*/

-- Query 2
SELECT empid, COUNT(*) AS numorders  -- Select the empid column and count the number of orders for each employee
FROM Sales.Orders                    -- From the Sales.Orders table
GROUP BY empid                       -- Group the results by empid
HAVING MAX(orderdate) < '20160501';  -- Only include employees whose latest order was before May 1st, 2016
